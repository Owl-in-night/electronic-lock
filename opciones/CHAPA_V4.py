    ...
    if id == "2":
        ...
        try:
            ...
            while True:
                ...
                # Si el switch esta encendido, encender el LED
                if button_state:
                    GPIO.output(led_pinA, GPIO.HIGH)
                    return jsonify({'status-led-A': "true"})
                # Si el switch esta apagado, apagar el LED
                else:
                    GPIO.output(led_pinA, GPIO.LOW)
                    time.sleep(0.1)
                    return jsonify({'status-led-A': "false"})
        ...
        try:
            ...
            while True:
                ...
                if button_state:
                    ...
                    return jsonify({'status-led-B': "true"})
                # Si el switch esta apagado, apagar el LED
                else:
                    GPIO.output(led_pinB, GPIO.LOW)
                    time.sleep(0.1)
                    return jsonify({'status-led-B': "false"})
        except KeyboardInterrupt:
            # Limpiar los pines GPIO y terminar el programa
            GPIO.cleanup()
